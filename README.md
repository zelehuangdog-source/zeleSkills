...
| [agent-session-restore](./agent-session-restore/) | 重启电脑前后，存档/恢复 Warp 里所有 agent 会话（Claude Code + Grok，含真实分屏布局），每个格子自动接回原来的 session | 原创 |
| [zele-code-review](./zele-code-review/) | 严格对照个人全局代码规范审查当前改动，后台跑 review 不阻塞，完成后逐条确认、可当场修复 | 原创 |
| [test-full](./test-full/) | 写完代码并本地部署后测本次改动：运行时选测试强度（轻/中/重）与是否做前后端联调，调本地服务接口走真实链路；执行前先做全量可执行性预检（跑不了的用例必须经用户确认才可跳过），缺数据就 INSERT 造数（加标记不删） | 原创 |
| [translate-article](./translate-article/) | 翻译外文文章到中文，自动添加摘要，本地化图片后存入 Obsidian 对应分类，并同步创建学城文档（含图片上传）；图注统一按论文式排版 | 原创 |
