...
（图的标题写在 .puml 的 `title` 里、跟着图走，这里不再另写图题）
...
- `title` 紧跟在固定头之后 —— **三类图都必须写**，格式见 § 3.2 的「必写：图标题」
...
- 标签里可以用 `\n` 换行
- **必须带标题**（`title`）：说明这张图覆盖的是谁 / 哪片范围。拆成多张时不写标题，就分不出哪张是哪个参与者的
...
图在 md 里只以图片形式出现（`![[diagrams/序列图-{用例名}.svg]]`），标题不写进图里就是没有标题 ——
**读者翻到第二张时，已经记不得第一张讲的是哪条流程**。

所以标题要写进图里（PlantUML `title`），**跟着图走**：图被单独复制、贴进评审材料时标题也还在。md 里不再另写图题。

| 项 | 要求 |
|---|---|
| 格式 | `{谁} + {做什么}`，一句业务动作描述 |
| 判据 | 只读这一行，就知道这条流程在解决什么事 |
| 写法 | 写在固定头之后、`autonumber` 之前；过长用 `\n` 换行 |
...
- [ ] 图里有标题？标题是「谁 + 做什么」的业务动作，不是「流程一」「序列图2」这种代号？
- [ ] 标题里没有类名、表名、接口路径、字段名？
- [ ] 每条泳道都在系统边界外？本系统只占一条？
...
- **必须带标题**（`title`）：一张图只覆盖一片范围时，标题要让人知道这片范围是什么，而不是只写「数据模型」
...
- **整体继承父字段的增量语义**：父字段新增 → 整个内嵌结构新增 → 整体填绿 `#C8E6C9`
- 父字段变更、内部只有部分子字段变 → 内嵌结构内部**逐字段标**（同字段级规则）
- 存量内嵌结构 → 无色

> ⚠️ 实测踩过：父字段 `config_json : JSON «新增»` 标了绿底，但它展开的虚线框是无色的，
> 里面列了二十来个字段 —— 看图根本分不出新旧，等于白标。
>
> 写法：内嵌结构也声明成 `entity`，直接加填充色：
> `entity "Topic 配置结构\n（内嵌，非独立节点）" as inner #C8E6C9 { ... }`

**4. 颜色之外必须有文字标记**：`<<新增>>` / `<<修改>>` / `<<删除>>`。
颜色不能单打独斗 —— 打印成黑白、或色弱的人看，颜色就失效了。

**5. 必须带图例**，用 PlantUML 的 `legend` 生成，图走到哪图例跟到哪。
图例里要说明虚线框的含义（内嵌结构，非独立节点）。
...
```plantuml
title Mafka 逻辑 Topic 的数据模型（只含本次变更相关的表）
