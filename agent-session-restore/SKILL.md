...
- 存档目录（每份存档一个子目录，恢复时读取）：`~/.claude/skills/agent-session-restore/snapshots/<snapshot-id>/`
  ...
  - `meta.txt`：第 1 行存档时间、第 2 行会话数、第 3 行 tab 数、第 4 行**存档构成**（`claude X / grok Y`，各占多少个格子，存档时写死）
...
6. **一个格子里有多个会话**：分四种，处理方式不同（判定集中在 `split_nested()`）。
   ...
   - **同一个 pid 记在多个会话上**（一个进程只有一个终端画面，登记表却能把一个 pid 挂到多个 session_id 下）：只保留排序最靠前的那个，其余跳过。这种是实测见过的：同一个 pid 会同时挂在两个 session_id 下。
